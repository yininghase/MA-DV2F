...
def inference_others(config):
    ...
    for i in tqdm(range(config["simulation runs"])):
        ...
        algorithm_name = config["run other algorithm"]
            
        config["starts"] = starts
        config["targets"] = targets
        config["obstacles"] = obstacles
        config["name"] = f"{algorithm_name}_vehicle={num_vehicles}_obstacle={num_obstacles}_run={i}"
        config["num of vehicles"] = num_vehicles
        config["num of obstacles"] = num_obstacles
        
        start_time = time.time()
        X_data, batches_data, _, y_model_data, success, num_step = sim_run(config)
        end_time = time.time()
...
if __name__ == "__main__":     
    ...
    parser.add_argument('--config_path', type=str, default=f"{WORK_SPACE}/configs/inference_cpu.yaml", help='specify configuration path')
    ...
    config["plot folder"] = os.path.join(config["plot folder"], algorithm_name)
    config["data folder"] = os.path.join(config["data folder"], algorithm_name)
